/* script.js - Logic for NC30 Demo (V16 - Fixes & PTZ Logic) */
...
document.addEventListener('DOMContentLoaded', () => {
    // Only init if not login page (unlikely in this SPA, but good practice)
    // Actually init handled by doLogin for this flow
});
...
function doLogin() {
    ...
    setTimeout(() => {
        ...
        document.getElementById('app-shell').style.display = 'grid'; // DIRECT ENTRY
    ...
    if(el) { 
        ...
        if (!sourceId) {
            ...
            if (sourceObj && (sourceObj.status === 'offline' || sourceObj.status === 'error')) {
                // Invalid Source (Signal Lost)
                if(headerBtn) headerBtn.disabled = false; // Allow opening panel to see error status
                updatePTZPanelInfo(windowNum, null, true); // Show Error Text
...
function updatePTZButtonState() {
    // Helper to ensure button state matches current selection (called after list refresh etc)
    const btn = document.getElementById('btn-ptz-ctrl');
    if(!btn) return;
    const selectedSlot = document.querySelector('.preview-slot.selected-slot');
    if (selectedSlot) {
        // Re-trigger selection logic to update button
...
// ... (Other standard functions Modal/Drag/Switch remain largely same) ...
...
function renderSourceModal(title, name, ip, group) {
    ...
    box.innerHTML = `
        <div class="modal-header-row"><h3 style="margin:0; color:#fff; font-size:16px;">${title}</h3><span class="modal-close-x" onclick="closeModal()">✕</span></div>
...
function openAutoSearch() {
    ...
    devices.forEach(d => {
        const el = document.createElement('div'); el.className = 'search-list-item'; el.innerText = `${d.ip} (${d.name})`;
...
function drop(ev) {
    ev.preventDefault(); const slot = ev.currentTarget; slot.classList.remove('drag-over');
    const data = JSON.parse(ev.dataTransfer.getData("application/json"));
    slot.dataset.sourceId = data.id; const windowNum = slot.id.split('-')[1];
    if (data.status === 'offline') { slot.classList.add('offline-state'); slot.innerHTML = `<div class="slot-label">Window ${windowNum}</div><div class="offline-overlay"><div class="offline-icon">⚠️</div><div class="offline-text">Signal Lost</div></div>${renderSlotMenu(slot.id)}`; } 
    ...
    slot.classList.add('active-slot'); selectSlot(slot.id); updateLiveHeader();
}
...
function updateHomeUI() { document.querySelectorAll('.mode-switch-item').forEach(btn => btn.classList.remove('active')); if (currentSystemMode === 'encoder') document.getElementById('top-btn-enc').classList.add('active'); else document.getElementById('top-btn-dec').classList.add('active'); }
function enterView(mode) {
    document.getElementById('view-encoder').style.display = 'none'; document.getElementById('view-decoder').style.display = 'none';
    ...
    if(mode === 'decoder') { document.getElementById('view-decoder').style.display = 'block'; renderSourceList(); updateLiveHeader(); selectSlot('slot-1'); }
    else { document.getElementById('view-encoder').style.display = 'block'; }
}
