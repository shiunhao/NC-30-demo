...
function renderSourceList() {
    ...
    sourcesData.forEach(src => {
        ...
        tr.innerHTML = `
            <td class="drag-col" style="padding-left:10px;">
            ...
            <td style="text-align:right; vertical-align:middle; padding-right:5px;">${statusHtml}</td>
        `;
    ...
    if(tabId === 'source') {
        ...
        if (currentSystemMode === 'encoder') {
            bodyEl.innerHTML = `<div style="background:#332b00; border:1px solid #d4b106; color:#ffeeba; padding:10px; border-radius:4px;">⚠️ Source management is only available in <a href="#" onclick="switchSettingsTab('av-settings')" style="color:#FF9500; font-weight:bold;">Decoder Mode</a>.</div>`;
             ...
             <table class="source-list-table" style="width: auto; min-width: 800px;"><thead class="source-list-header"><tr><th>Preview</th><th>Name & IP</th><th>Status</th><th style="text-align:right;">Actions</th></tr></thead>
             <tbody>${sourcesData.map(s => `<tr><td style="padding:10px;"><div class="thumb-box"><img src="${s.thumb}"></div></td><td style="padding:10px;"><div style="color:#fff;">${s.type==='rtsp'?'<span style="background:#555;color:#fff;font-size:10px;padding:2px 4px;border-radius:3px;margin-right:5px;">RTSP</span>':''}${s.name}</div><div style="font-size:12px;color:#888; max-width: 250px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" title="${s.ip}">${s.ip}</div></td><td style="padding:10px;color:${s.status==='online'?'#4CAF50':'#888'}">${s.status.toUpperCase()}</td><td style="text-align:right;padding:10px;"><button class="btn btn-outline btn-sm" onclick="editSource('${s.id}')">Edit</button> <button class="btn btn-danger btn-sm" onclick="askRemoveSource('${s.id}')">Delete</button></td></tr>`).join('')}</tbody></table>
             </div>`;
        }
    } 
