/* script.js - Logic for NC30 Demo (V20 - Final & Complete) */
...
function removeSource(sid, srcId) { if(srcId) { const s = document.querySelector(`.preview-slot[data-source-id="${srcId}"]`); if(s) sid = s.id; else return; } const slot = document.getElementById(sid); if(slot) { delete slot.dataset.sourceId; slot.classList.remove('active-slot', 'offline-state'); slot.innerHTML = `<div class="slot-label">Window ${sid.split('-')[1]}</div><div class="slot-content" style="text-align:center;"><div class="slot-name" style="color:#555;">[ Drag Source Here ]</div></div>`; updateLiveHeader(); selectSlot(sid); renderSourceList(); } }
...
// FIX 3: Restore Global Settings Content
