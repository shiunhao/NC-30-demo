/* script.js - Final Demo Version (V72 - UI Refined, Reordered Modal, New Side Menu) */
...
function doLogin() {
    ...
    setTimeout(() => { 
        ...
        const hasOnboarded = localStorage.getItem('nc30_onboarding_v72');
        if (!hasOnboarded) {
            startOnboarding();
        } else {
...
function closeOnboarding() {
    localStorage.setItem('nc30_onboarding_v72', 'true');
...
// ... (Account Functions omitted for brevity, same as V71)
...
// === NEW: switchSettingsTab with New Sidebar Structure ===
function switchSettingsTab(tabId) {
    // 1. Update Sidebar Active State
    // Re-render sidebar to match new structure
    const sidebar = document.querySelector('.settings-sidebar');
    sidebar.innerHTML = `
        <div class="sidebar-item ${tabId==='source'?'active':''}" onclick="switchSettingsTab('source')" id="tab-source">Source</div>
        <div class="sidebar-item ${tabId==='av-settings'?'active':''}" onclick="switchSettingsTab('av-settings')" id="tab-av-settings">Video & Audio</div>
        <div class="sidebar-item ${tabId==='ndi'?'active':''}" onclick="switchSettingsTab('ndi')" id="tab-ndi">NDI</div>
        <div class="sidebar-item ${tabId==='network'?'active':''}" onclick="switchSettingsTab('network')" id="tab-network">Network</div>
        <div class="sidebar-item ${tabId==='system'?'active':''}" onclick="switchSettingsTab('system')" id="tab-system">System</div>
        <div class="sidebar-item ${tabId==='help'?'active':''}" onclick="switchSettingsTab('help')" id="tab-help">Help</div>
    `;
    ...
    // Default Header
    let headerContent = `<span class="settings-title" id="settings-title">Settings</span><span class="settings-close-btn" onclick="closeSpecificModal('modal-large-settings')">✕</span>`;

    // Tab Content Logic
    if(tabId === 'source') {
        // Custom Header for Source with Tour Button on Right
        headerContent = `
        ...
        headerContent = `<span class="settings-title">${title}</span><span class="settings-close-btn" onclick="closeSpecificModal('modal-large-settings')">✕</span>`;
        ...
        headerContent = `<span class="settings-title">NDI Settings</span><span class="settings-close-btn" onclick="closeSpecificModal('modal-large-settings')">✕</span>`;
        ...
        headerContent = `<span class="settings-title">Network Settings</span><span class="settings-close-btn" onclick="closeSpecificModal('modal-large-settings')">✕</span>`;
        ...
        headerContent = `<span class="settings-title">System Information</span><span class="settings-close-btn" onclick="closeSpecificModal('modal-large-settings')">✕</span>`;
        ...
        headerContent = `<span class="settings-title">Help</span><span class="settings-close-btn" onclick="closeSpecificModal('modal-large-settings')">✕</span>`;
        bodyEl.innerHTML = `<div class="settings-subsection"><p>For more information, visit <a href="#" style="color:var(--theme-color)">aver.com</a></p><button class="btn btn-primary" onclick="startOnboarding()">Restart Feature Tour</button></div>`;
    }
...
// === RENDER SOURCE MODAL (Reordered: Group -> IP -> Name -> Remark) ===
...
// ... (Source Tour / Auto Search logic remains V71)
function generateSourceTourOverlay() {
    ...
    div.innerHTML = `
        <div class="onboarding-box">
            ...
            <div class="source-tour-step" id="tour-step-1" style="display:block;"><div class="onboarding-icon">➕</div><h2>1. Add a New Source</h2><p>Click the <strong>"+ Add Source"</strong> button to manually enter an IP address or use Auto Search to find NDI sources on your network.</p><div class="onboarding-actions"><button class="btn btn-primary" onclick="nextSourceTour(2)">Next</button></div></div>
            <div class="source-tour-step" id="tour-step-2" style="display:none;"><div class="onboarding-icon">🖱️</div><h2>2. Manage Sources</h2><p>In <strong>Decoder Mode</strong>, simply drag and drop any source from the list on the right into the preview window to start monitoring.</p><div class="onboarding-actions"><button class="btn btn-outline" onclick="nextSourceTour(1)">Back</button><button class="btn btn-primary" onclick="nextSourceTour(3)">Next</button></div></div>
            <div class="source-tour-step" id="tour-step-3" style="display:none;"><div class="onboarding-icon">🔄</div><h2>3. Switch Modes</h2><p>Need to transmit? Go to the <strong>"Video & Audio"</strong> tab in Settings to switch this device to <strong>Encoder Mode</strong>.</p><div class="onboarding-actions"><button class="btn btn-outline" onclick="nextSourceTour(2)">Back</button><button class="btn btn-primary" onclick="closeSourceTour()">Finish</button></div></div>
        </div>
    `;
...
function openAutoSearch() { const modal = document.getElementById('modal-auto-search'); const list = document.getElementById('search-list-content'); const currentScope = document.getElementById('inputSrcGroup').value || "Public"; list.innerHTML = `<div style="padding:15px; border-bottom:1px solid #333; text-align:center;"><div style="color:#fff; font-weight:bold; margin-bottom:5px;">Scanning Scope: <span style="color:#007AFF">${currentScope}</span></div><div style="color:#888; font-size:12px;">Searching for NDI sources in ${currentScope}...</div></div><div id="found-devices-list" style="margin-top:10px;"><div style="color:#888; text-align:center; padding:20px;"><div class="spinner-ring" style="margin:0 auto 10px auto;"></div>Scanning...</div></div>`; modal.style.display = 'flex'; setTimeout(() => { const container = document.getElementById('found-devices-list'); if(container) { container.innerHTML = ''; const devices = [ { ip: '192.168.1.101', name: 'Camera 01' }, { ip: '192.168.1.105', name: 'PTZ Cam' }, { ip: '192.168.1.200', name: 'PC Stream' }, { ip: '192.168.1.205', name: 'Meeting Room' } ]; devices.forEach(d => { const el = document.createElement('div'); el.className = 'search-list-item'; el.innerText = `${d.ip} (${d.name})`; el.onclick = function() { document.getElementById('inputSrcIP').value = d.ip; document.getElementById('inputSrcName').value = d.name; closeAutoSearch(); }; container.appendChild(el); }); } }, 1000); }
